class StackedAreaChart {
    ...
    initVis() {
        ...
        vis.stack = d3.stack().keys(["west", "south", "northeast", "midwest"]);
    ...
    wrangleData() {
        ...
        vis.dataFiltered = vis.dayNest.map((day) =>
            day.values.reduce(
                (accumulator, current) => {
    ...
    updateVis() {
        ...
        vis.teams = vis.g.selectAll(".team").data(vis.stack(vis.dataFiltered));

        // update the path for each team
        vis.teams.select(".area").attr("d", vis.area);
