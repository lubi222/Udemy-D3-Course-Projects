class BarChart {
    ...
    initVis() {
        ...
        vis.linePath = vis.g.append("path").attr("class", "line").attr("fill", "none").attr("stroke-width", "3px");
        ...
        vis.xAxisCall = d3.axisBottom().tickFormat((d) => "" + capitalizeFirstLetter(d));
    ...
    wrangleData() {
        ...
        vis.dataFiltered = nestedCalls.map((category) => {
            return {
                ...
                size:
                    category.values.reduce((accumulator, current) => accumulator + current[vis.variable], 0) /
    ...
    updateVis() {
        ...
        vis.rects = vis.g.selectAll("rect").data(vis.dataFiltered, (d) => d.category);
